...
用于清理超鹿运动 App 中的开屏、Banner、营销卡片、运营弹窗和推广图片，支持 Loon、Egern 和 Surge。

## 功能与匹配依据

经网络流量分析，相关运营位由 `app.chaolu.com.cn` 的独立接口返回；部分推广图片位于 `img.chaolu.com.cn` 的稳定活动目录。插件只处理下列接口和图片路径，不阻断整个业务域名或图片 CDN。
...
图片请求仅在主机为 `img.chaolu.com.cn`，且路径包含以下任一关键字时被拦截：

- `invite-gift`
- `88sale2026`
- `paymentSuccessPromo`

Loon 和 Egern 对命中的请求返回图片拒绝响应；Surge 使用 URL Rewrite 拒绝请求。规则不匹配 `img.chaolu.com.cn` 上不含这些关键字的课程、教练和其他普通图片。
...
所有目标接口和图片均使用 HTTPS，响应体过滤及图片路径改写需要安装并信任所用客户端的 CA 证书，并开启 MITM。配置仅将 `app.chaolu.com.cn` 和 `img.chaolu.com.cn` 加入 MITM 主机列表；Surge 同时将这两个主机加入 HTTP engine 列表。
...
- `invite-gift` 和 `88sale2026` 活动目录还可能承载邀新页面的按钮、空状态等界面素材；关键词规则会一并拦截，打开相关活动页时可能出现缺图。
